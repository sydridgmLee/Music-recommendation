...
sc = SparkContext("local", "user_artist_rate")
...
user_artist_rate_RDD = user_item_rate_class_RDD.map(lambda line: line.split(",")).\
    filter(lambda x: int(x[3]) == 3).\
    map(lambda tokens: (tokens[0],tokens[1], float(tokens[2]))).cache()
#-------------------------------------------------------------------

training_RDD, validation_RDD, test_RDD = user_artist_rate_RDD.randomSplit([18, 1, 1], seed=3L)
...
user_artist_for_rec_RDD = sc.textFile(user_artist_for_rec_file).\
    map(lambda line: line.split(",")).map(lambda tokens: (tokens[0], tokens[1]))

model = ALS.train(user_artist_rate_RDD, best_rank, seed=3, iterations=50)

predictions = model.predictAll(user_artist_for_rec_RDD).\
...
dir = 'data/predictions_artist/'
output(predictions, dir)
writeCSV(dir, "data/predictions_user_artist.csv")

dir = 'data/user_rate_mean_artist/'
output(mean_rating, dir)
writeCSV(dir, "data/predictions_user_artist_mean.csv")
