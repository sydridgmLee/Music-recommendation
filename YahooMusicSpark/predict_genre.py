...
sc = SparkContext("local", "user_genre_rate")
...
user_genre_rate_RDD = user_item_rate_class_RDD.map(lambda line: line.split(",")).\
    filter(lambda x: int(x[3]) == 4).\
    map(lambda tokens: (tokens[0],tokens[1], float(tokens[2]))).distinct().cache()

training_RDD, validation_RDD, test_RDD = user_genre_rate_RDD.randomSplit([18, 1, 1], seed=3L)
...
#=======> very important __distinct__
user_genre_for_rec_RDD = sc.textFile(user_genre_for_rec_file).\
    map(lambda line: line.split(",")).map(lambda tokens: (tokens[0], tokens[1])).distinct()

model = ALS.train(user_genre_rate_RDD, rank=best_rank, seed=3, iterations=50)

predictions = model.predictAll(user_genre_for_rec_RDD).\
...
dir = 'data/predictions_genre/'
output(predictions, dir)
writeCSV(dir, "data/predictions_user_genre.csv")

dir = 'data/user_rate_mean_genre/'
output(mean_rating, dir)
writeCSV(dir, "data/predictions_user_genre_mean.csv")
